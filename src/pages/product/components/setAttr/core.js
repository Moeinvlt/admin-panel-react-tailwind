...
import { getCategoryAttrsApi } from "../../../../api/category/categoryAttr";
import { addProductAttrApi } from "../../../../api/products/productsAtrrsApi";
...
export const onSubmit = async (values, actions, productId) => {
  ...
  const res = await addProductAttrApi(productId, data);
  if (res.status === 200) {
...
export const initializingData = async (selectedProduct) => {
  ...
  const promise = Promise.all(
    selectedProduct.categories.map(async (cat) => {
      const res = await getCategoryAttrsApi(cat.id);
      if (res.status === 200) {
        attrsVar = [
          ...attrsVar,
