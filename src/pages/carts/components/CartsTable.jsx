...
const CartsTable = () => {
  ...
  const hasAddCartPerm = useHasPermission("create_cart");
  const hasActionPerm = useHasPermission(["update_cart", "delete_cart"]);
  ...
  const dataInfo = useMemo(() => {
    const baseColumns = [
      ...
      },
      {
        field: null,
        title: "تعداد کالاها",
        elements: (rowData) => rowData.items?.length || 0,
      },
      {
        field: null,
        title: "وضعیت فعال",
  ...
  return (
    <>
      <PaginatedDataTable
        ...
        modalBtn={false}
        addPageBtn={
          hasAddCartPerm ? <ModalPageBtn linkPath="/carts/add-cart" /> : false
        }
      />
      <Outlet context={{ setCartsData }} />
    </>
  );
};
